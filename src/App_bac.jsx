...
`;
    ...
    const callGeminiTextLLM = useCallback(
        async (userPrompt, systemPromptToUse) => {
            ...
            const backup1ApiKey = 'AIzaSyAhscNjW8GmwKPuKzQ47blCY_bDanR-B84';
            const backup2ApiKey = 'AIzaSyCH-v67rjijFO_So2mTDj_-qIy2aNJYgz0';
            ...
            try {
                ...
                if (!response.ok) response = await tryGeminiCall(backup1ApiKey);
                if (!response.ok) response = await tryGeminiCall(backup2ApiKey);
...
### 중요 규칙
- **한국어 사용:** 모든 서사('personalStory', 'publicLogEntry' 등)와 선택지('choices')는 반드시 한국어로 작성되어야 합니다. 영어 또는 다른 언어 사용은 금지됩니다.
